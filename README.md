...
* User votes now persist between page reloads. 
    * Because the logged in user changes when you reload the page it will take a few reloads to see the page for the same user.  
    * Example: User 1 votes for a book "Famous Cats of the Internet" but no one else has.  On page reload the page displays as User 2.  User 2 will see the effect of User 1's votes, but is not considered to have voted for it.  Another reload will do the same as User 3.  One more reload and the page displays as User 1, and they are unable to vote for Famous Cats of the Internet again
* When a recommendation is initially entered, it records the user id that created it.  This is displayed on the UI for all users looking at that recommendation.
